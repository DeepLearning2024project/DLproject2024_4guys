\section{Data Set}
\label{sec:dataset}
%本实验的数据集由小组成员采集于工业和信息化部电子第五研究所，采集对象包括六个同种的电子元器件，记录持续时间内测试电子元件的电流大小与时间。本实验的训练集为所采集的四号电子原件，包含1058条数据。其他数据集用于测试和验证，由于最后所预测的是以天数为单位的电子元件寿命而原始数据集在一天内多次测量电流，因此所有用于测试的数据集按照日期进行划分。
